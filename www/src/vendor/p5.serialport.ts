...
class SerialPort {
  ...
  listCallback: (arg0: any) => void;
  connectedCallback: () => void;
  rawDataCallback: (arg0: any) => void;

  constructor(_hostname?: string, _serverport?: number) {
    var self = this;
   ...
   *
   */
  emit(data: { method: string; data: any }) {
   ...
   * 		}
   */
  list(cb: (arg0: any) => void): Array<any> {
   ...
   *		}
   */
  open(_serialport: string, _serialoptions?: SerialOptions, cb?: () => void) {
    if (typeof cb === "function") {
   ...
   *		charToFind.charCodeAt(0, );
   */
  readBytesUntil(charToFind: string): number[] | number {
   ...
   *		 }
   */
  readStringUntil(stringToFind: string): string {
    var stringBuffer = [];
    //console.log("serialBuffer Length: " + this.serialBuffer.length);
    for (var i = 0; i < this.serialBuffer.length; i++) {
      //console.log("push: " + String.fromCharCode(this.serialBuffer[i]));
    ...
    let string = stringBuffer.join("");
    //console.log("stringBuffer: " + stringBuffer);

    var returnString = "";
    var foundIndex = string.indexOf(stringToFind);
    //console.log("found index: " + foundIndex);
    if (foundIndex > -1) {
      returnString = string.substr(0, foundIndex);
      this.serialBuffer = this.serialBuffer.slice(
   ...
   *
   */
  onData(_callback: () => void | ((arg0: any) => void)) {
    this.on("data", _callback);
  }

  onOpen(_callback: () => void | ((arg0: any) => void)) {
    this.on("open", _callback);
  }

  onClose(_callback: () => void | ((arg0: any) => void)) {
    this.on("close", _callback);
  }

  onError(_callback: () => void | ((arg0: any) => void)) {
    this.on("error", _callback);
  }

  onList(_callback: () => void | ((arg0: any) => void)) {
    this.on("list", _callback);
  }

  onConnected(_callback: () => void | ((arg0: any) => void)) {
    this.on("connected", _callback);
  }

  onRawData(_callback: () => void | ((arg0: any) => void)) {
    this.on("rawdata", _callback);
  }

  // Version 2
  on(_event: string, _callback: () => void | ((arg0: any) => void)) {
    if (_event == "open") {
